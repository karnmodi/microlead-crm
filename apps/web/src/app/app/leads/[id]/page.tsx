...
type LeadEmailsRes = { data: LeadEmailRow[] };
...
export default function LeadDetailPage() {
  ...
  const emails = useQuery({
    ...
    queryFn: () => api<LeadEmailsRes>(`/leads/${id}/emails`),
    enabled: !!id,
  });
  ...
  const sendDraftEmail = useMutation({
    ...
    onSuccess: () => {
      void runAi("summary");
      void qc.invalidateQueries({ queryKey: ["activities", "LEAD", id] });
      void qc.invalidateQueries({ queryKey: ["lead-emails", id] });
    },
  });
        ...
        <div className="space-y-4">
          ...
          <section className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500">{emails.data?.data.length ?? 0} sent</span>
            </div>
            <ul className="mt-3 space-y-2">
              {emails.data?.data.length === 0 && (
                <li className="rounded-lg border border-dashed border-zinc-200 px-4 py-6 text-center text-sm text-zinc-400 dark:border-zinc-700">
                  No emails sent yet.
                </li>
              )}
              {emails.data?.data.map((em) => {
                const isOpen = expandedEmailId === em.id;
                return (
                  <li key={em.id} className="rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setExpandedEmailId(isOpen ? null : em.id)}
                      className="flex w-full items-start justify-between gap-3 px-3 py-2.5 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 truncate text-xs text-zinc-500">
                          To: {em.emailTo} · {em.actor.name ?? em.actor.email}
                        </p>
                      </div>
                      <div className="flex shrink-0 items-center gap-2">
                        <span className="whitespace-nowrap text-xs text-zinc-400">
                          {compactDateTime(em.sentAt)}
                        </span>
                        <svg
                          viewBox="0 0 24 24"
                    ...
                    {isOpen && (
                      <div className="border-t border-zinc-100 bg-zinc-50 px-3 py-3 dark:border-zinc-800 dark:bg-zinc-950">
